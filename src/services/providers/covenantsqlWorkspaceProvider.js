...
export default new Provider({
  ...
  async initWorkspace() {
    ...
    if (!store.getters['workspace/workspacesById'][workspaceId]) {
      try {
        ...
        const db = await covenantsqlHelper.getDb(store.getters['data/couchdbTokensBySub'][workspaceId]);
  ...
  async getChanges() {
    ...
    const result = await covenantsqlHelper.getChanges(syncToken, lastSeq);
  ...
  async saveWorkspaceItem({ item, syncData }) {
    const syncToken = store.getters['workspace/syncToken'];
    const { id, rev } = await covenantsqlHelper.uploadDocument({
      token: syncToken,
      item,
  ...
  removeWorkspaceItem({ syncData }) {
    const syncToken = store.getters['workspace/syncToken'];
    return covenantsqlHelper.removeDocument(syncToken, syncData.id, syncData.rev);
  },
  async downloadWorkspaceContent({ token, contentSyncData }) {
    const body = await covenantsqlHelper.retrieveDocumentWithAttachments(token, contentSyncData.id);
  ...
  async downloadWorkspaceData({ token, syncData }) {
    ...
    const body = await covenantsqlHelper.retrieveDocumentWithAttachments(token, syncData.id);
  ...
  async uploadWorkspaceContent({ token, content, contentSyncData }) {
    const res = await covenantsqlHelper.uploadDocument({
      token,
      item: {
        id: content.id,
        type: content.type,
  ...
  async listFileRevisions({ token, contentSyncDataId }) {
    const body = await covenantsqlHelper.retrieveDocumentWithRevisions(token, contentSyncDataId);
    const revisions = [];
  ...
  async loadFileRevision({ token, contentSyncDataId, revision }) {
    ...
    const body = await covenantsqlHelper.retrieveDocument(token, contentSyncDataId, revision.id);
  ...
  async getFileRevisionContent({ token, contentSyncDataId, revisionId }) {
    const body = await covenantsqlHelper
